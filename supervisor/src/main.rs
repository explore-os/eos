...
async fn check_props(
    ...
    let mut entries = tokio::fs::read_dir(&props_dir).await?;
    while let Some(entry) = entries.next_entry().await? {
        ...
        let content = tokio::fs::read_to_string(&props_file).await?;
        ...
        tokio::fs::remove_file(props_file).await?;
    }
    Ok(())
}
...
async fn check_actors(actor_dir: impl AsRef<Path>, tick_speed: u64) -> anyhow::Result<()> {
    let mut entries = tokio::fs::read_dir(&actor_dir).await?;
    while let Some(entry) = entries.next_entry().await? {
        ...
        if move_next_message(&message_dir, tick_speed).await? {
            ...
            let pid = tokio::fs::read_to_string(actor_dir.join(".pid"))
                .await?
                .parse::<usize>()?;
            nix::sys::signal::kill(
...
async fn kill_actors(actor_dir: impl AsRef<Path>) -> anyhow::Result<()> {
    ...
    while let Some(entry) = entries.next_entry().await? {
        ...
        let pid_string = tokio::fs::read_to_string(entry.path().join(".pid")).await?;
        ...
        for _ in 0..30 {
            if !tokio::fs::try_exists(Path::new("/proc").join(format!("{pid}"))).await? {
                break;
            }
...
async fn check_queue(
    ...
    let mut entries = tokio::fs::read_dir(&send_dir).await?;
    let mut messages = Vec::new();
    while let Some(entry) = entries.next_entry().await? {
        if entry.path().is_dir() {
            continue;
    ...
    for (msg_path, t) in messages {
            ...
            .split_once("::")
        {
            ...
            tokio::fs::rename(&msg_path, &target_dir.join(id)).await?;
        }
    }
    Ok(())
}
